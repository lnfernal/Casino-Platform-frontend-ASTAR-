...
export default function Provider({ children }) {
    ...
    const checkBalance = async () => {
        try {
            if (wallet.status === "connected") {
                const balance = await ICICBContract.balanceOf(wallet.account);
                dispatch({
                    type: "balance",
    ...
    const checkApproval = async (spenderAdddress) => {
        if (wallet.status === "connected") {
            const signedICICBContract = ICICBContract.connect(state.signer);
            const allowance = await signedICICBContract.allowance(
                wallet.account,
                spenderAdddress
            );
        ...
        try {
            if (wallet.status === "connected") {
                ...
                if (balance >= amount) {
                    ...
                    if (allowance < amount) {
                        const signedICICBContract = ICICBContract.connect(
                            state.signer
                        );
                        var tx = await signedICICBContract.approve(
                            treasuryContract.address,
    ...
    const getStakingPoolInfo = async (poolAddress) => {
        if (wallet.status === "connected")
            try {
                ...
                const poolBalancePromise = ICICBContract.balanceOf(
                    stakingPool.address
                );
